...
Solver<Dtype>::Solver(const string& param_file)
  ...
  LOG(INFO) << "Has read proto from text file" << std::endl << param.DebugString();
  Init(param);
}
...
void Solver<Dtype>::Init(const SolverParameter& param) {
  ...
  // Scaffolding code
  LOG(INFO) << "Start initializing train net" << std::endl << param.DebugString();
  InitTrainNet();
  LOG(INFO) << "Start initializing test net" << std::endl << param.DebugString();
  InitTestNets();
...
void SGDSolver<Dtype>::PreSolve() {
  ...
  for (int i = 0; i < net_params.size(); ++i) {
    ...
    history_.push_back(shared_ptr<Blob<Dtype> >(new Blob<Dtype>(
        ...
        net_param->width())));
    history2_.push_back(shared_ptr<Blob<Dtype> >(new Blob<Dtype>(
        net_param->num(), net_param->channels(), net_param->height(),
        net_param->width())));
...
void AdamSolver<Dtype>::ComputeUpdateValue() {
  ...
  if (this->iter_ < 10000) {
    rate = rate * sqrt(1 - pow(1-beta2, this->iter_)) / (1 - pow(1-beta1, this->iter_));
  }
  i*/
  ...
  case Caffe::CPU:
    for (int param_id = 0; param_id < net_params.size(); ++param_id) {
      ...
      // compute biased first moment (history_)
      // m = m + b1*(grad - m)
      caffe_cpu_axpby(net_params[param_id]->count(),
          beta1, net_params[param_id]->cpu_diff(),
          Dtype(1.0)-beta1, this->history_[param_id]->mutable_cpu_data());
      
      // compute biased second row monent (temp_)
      // v = v + b2*(grad^2 - v)
      caffe_powx(net_params[param_id]->count(),
          net_params[param_id]->cpu_diff(), Dtype(2),
          net_params[param_id]->mutable_cpu_diff());
      ...
      caffe_cpu_axpby(net_params[param_id]->count(),
          beta2, net_params[param_id]->cpu_diff(),
          Dtype(1.0)-beta2, this->history2_[param_id]->mutable_cpu_data());
          
      // prepare update
      // dw = m / (sqrt(v) + e)
      caffe_powx(net_params[param_id]->count(),
                this->history2_[param_id]->cpu_data(), Dtype(0.5),
                this->update_[param_id]->mutable_cpu_data());
  ...
  case Caffe::GPU:
    ...
    for (int param_id = 0; param_id < net_params.size(); ++param_id) {
      ...
      caffe_gpu_axpby(net_params[param_id]->count(),
          beta1, net_params[param_id]->gpu_diff(),
          Dtype(1.0)-beta1, this->history_[param_id]->mutable_gpu_data());
      
      // compute biased second row monent (hostory2_)
      caffe_gpu_powx(net_params[param_id]->count(),
          net_params[param_id]->gpu_diff(), Dtype(2),
          net_params[param_id]->mutable_gpu_diff());

      caffe_gpu_axpby(net_params[param_id]->count(),
          beta2, net_params[param_id]->gpu_diff(),
          Dtype(1.0)-beta2, this->history2_[param_id]->mutable_gpu_data());
          
      // prepare update
      caffe_gpu_powx(net_params[param_id]->count(),
                this->history2_[param_id]->gpu_data(), Dtype(0.5),
                this->update_[param_id]->mutable_gpu_data());
